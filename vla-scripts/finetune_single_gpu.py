"""
finetune_single_gpu.py

Fine-tunes OpenVLA via LoRA on single GPU (no distributed training).
Based on the original finetune.py but with all distributed training code removed.
"""
...
def init_module_single_gpu(
    ...
    """
    Initializes a module for single GPU training (no DDP wrapping).
    """
...
def save_training_checkpoint_single_gpu(
    cfg,
    run_dir,
    log_step,
    vla,
    ...
    """
    Save all training checkpoints for single GPU training.
    """
...
def run_validation_single_gpu(
    ...
    """
    Compute validation set metrics for logging (single GPU version).
    """
...
@draccus.wrap()
def finetune_single_gpu(cfg: FinetuneConfig) -> None:
    """
    Fine-tunes base VLA on demonstration dataset via LoRA using single GPU.
    All distributed training code has been removed.
    """
    ...
    print(f"Fine-tuning OpenVLA Model `{cfg.vla_path}` on `{cfg.dataset_name}` (Single GPU)")
    ...
    if cfg.use_proprio:
        proprio_projector = init_module_single_gpu(
            ProprioProjector,
            "proprio_projector",
            cfg,
            device_id,
    ...
    if cfg.use_l1_regression:
        action_head = init_module_single_gpu(
    ...
    if cfg.use_diffusion:
        ...
        noisy_action_projector = init_module_single_gpu(
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if gradient_step_idx > 0 and log_step % cfg.save_freq == 0:
                save_training_checkpoint_single_gpu(
                    cfg=cfg,
                    run_dir=run_dir,
                    log_step=log_step,
                    vla=vla,
